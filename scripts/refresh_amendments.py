...
def synchronize() -> dict:
    ...
    if not feed_xml:
        print("Flux RSS injoignable, abandon.", file=sys.stderr)
    ...
    # 3. Fetch les XML en parallèle
    enriched: dict[str, dict] = {}
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        futures = {ex.submit(fetch_and_parse_xml, it["num"], it["xml_url"]): it for it in feed_items}
    ...
    print(f"XML enrichis : {summary['fetched']}/{len(feed_items)}")
 
    # 4. Appliquer aux amendements
    for item in feed_items:
        ...
        if existing is None:
            ...
            # Mise à jour de l'état si différent
